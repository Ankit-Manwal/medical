...
function DiabetesTest({ name='Diabetes', onDone }) {
  ...
  const run = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/diabetes/predict', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
  ...
  return (
    <div className="card-panel">
      <h4 style={{ marginBottom: 8 }}>{name} Test Parameters</h4>
